...
export default function ConversationWelcome({
  ...
  useEffect(() => {
    let active = true;
    const load = () => api.get("/ticket-metrics/daily").then(({
      data
    }) => {
  ...
  return <section className={classes.root} aria-label="Resumo do atendimento">
      <div className={classes.content}>
        ...
        <div className={classes.stats}>
          {[["Em atendimento", counts.open, counts.open == null], ["Aguardando", counts.pending, counts.pending == null], ["Entradas de hoje", today, metricsLoading]].map(([label, value, loading]) => <Paper className={classes.stat} key={label}>
        ...
        <div className={classes.actions}>
          <Button color="primary" variant="contained" endIcon={<ArrowForwardIcon />} onClick={onViewQueue}>
            Ver fila de espera
          </Button>
          <Button variant="outlined" startIcon={<AddIcon />} onClick={() => setNewTicketOpen(true)}>
            Novo atendimento
          </Button>
        </div>
        <Typography className={classes.note} variant="caption" color="textSecondary">
